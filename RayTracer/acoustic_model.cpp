...
void AcousticModel::precomputeBandConstants()
{
    for (int i = 0; i < NUM_BANDS; ++i) {
        ...
        // Absorption atmosphérique [dB/m] par bande (ISO 9613-1)
        cached_alpha_[i] = absorptionCoefficient(f);
        // Impédance normalisée Delany-Bazley (indépendante de l'angle et de la distance)
...
void AcousticModel::precomputeLw()
{
    ...
    // Somme pondérée A pour l'estimation rapide
...
double AcousticModel::estimateSPL_fast(double distance_m) const
{
    ...
    // SPL ≈ Lw_total_A − divergence géométrique (pas d'absorption, pas de sol)
